...
class Courses(db.Model):
    ...
    unique_id = db.Column(db.String(5), nullable=False)
    ...
    def to_dict(self):
        return {"id": self.id, "unique_id": self.unique_id, "course_id": self.course_id, "course_name": self.course_name, "subject": self.subject, "instructor": self.instructor, "dates": self.dates, "time": self.time, "registered_students": self.registered_students, "capacity": self.capacity, "description": self.description}
...
def Create():
    x = request.json
    data = Courses(unique_id=GenerateUniqueID(), course_id=x['course_id'], course_name=x['course_name'], subject=x['subject'], dates=x['dates'], time=x['time'], capacity=x['capacity'], description=x['description'])
    db.session.add(data)
    db.session.commit()
